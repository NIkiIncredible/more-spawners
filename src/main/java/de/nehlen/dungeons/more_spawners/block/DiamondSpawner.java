...
public class DiamondSpawner extends BlockWithEntity implements BlockEntityProvider{
    public DiamondSpawner(Settings settings) {
        super(settings);
    }
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new DiamondSpawnerBlockEntity(pos, state);
    }

    @Override
    @Nullable
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return DiamondSpawner.checkType(type, BlockEntitiesManager.DIAMOND_SPAWNER_BLOCK_ENTITY, world.isClient ? DiamondSpawnerBlockEntity::clientTick : DiamondSpawnerBlockEntity::serverTick);
    }
    ...
    public void onStacksDropped(BlockState state, ServerWorld world, BlockPos pos, ItemStack tool, boolean dropExperience) {
        ...
        if (dropExperience) {
            int i = (15*3) + world.random.nextInt(15) + world.random.nextInt(15);
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (!world.isClient) {
            if (player.getMainHandStack().getItem() instanceof SpawnEggItem) {
                ...
                DiamondSpawnerBlockEntity diamondSpawnerBlockEntity = (DiamondSpawnerBlockEntity) world.getBlockEntity(pos);
                EntityType<?> entityType = item.getEntityType(itemStack.getNbt());

                diamondSpawnerBlockEntity.setEntityType(entityType, world.getRandom());
                diamondSpawnerBlockEntity.markDirty();
