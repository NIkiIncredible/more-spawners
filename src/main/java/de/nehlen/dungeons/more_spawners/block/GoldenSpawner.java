...
public class GoldenSpawner extends BlockWithEntity implements BlockEntityProvider{
    public GoldenSpawner(Settings settings) {
        super(settings);
    }
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new GoldenSpawnerBlockEntity(pos, state);
    }

    @Override
    @Nullable
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return GoldenSpawner.checkType(type, BlockEntitiesManager.GOLDEN_SPAWNER_BLOCK_ENTITY, world.isClient ? GoldenSpawnerBlockEntity::clientTick : GoldenSpawnerBlockEntity::serverTick);
    }
    ...
    public void onStacksDropped(BlockState state, ServerWorld world, BlockPos pos, ItemStack tool, boolean dropExperience) {
        ...
        if (dropExperience) {
            int i = (15*2) + world.random.nextInt(15) + world.random.nextInt(15);
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        return MoreSpawnerHelper.getActionResult(state, world, pos, player);
    }
}
