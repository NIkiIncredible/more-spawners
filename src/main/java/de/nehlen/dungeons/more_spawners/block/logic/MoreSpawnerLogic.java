...
public abstract class MoreSpawnerLogic {
    ...
    private DataPool<MobSpawnerEntry> spawnPotentials = DataPool.<MobSpawnerEntry>builder().build();
    @Nullable
    ...
    private boolean isPlayerInRange(World world, BlockPos pos) {
        return world.isPlayerInRange((double)pos.getX() + 0.5, (double)pos.getY() + 0.5, (double)pos.getZ() + 0.5, this.getRequiredPlayerRange());
    }
    ...
    public void serverTick(ServerWorld world, BlockPos pos) {
        ...
        for (int i = 0; i < this.getSpawnCount(); ++i) {
            ...
            int j = nbtList.size();
            double d = j >= 1 ? nbtList.getDouble(0) : (double)pos.getX() + (random.nextDouble() - random.nextDouble()) * (double)this.getSpawnRange() + 0.5;
            double e = j >= 2 ? nbtList.getDouble(1) : (double)(pos.getY() + random.nextInt(3) - 1);
            double d2 = f = j >= 3 ? nbtList.getDouble(2) : (double)pos.getZ() + (random.nextDouble() - random.nextDouble()) * (double)this.getSpawnRange()+ 0.5;
            ...
            int k = world.getNonSpectatingEntities(entity2.getClass(), new Box(pos.getX(), pos.getY(), pos.getZ(), pos.getX() + 1, pos.getY() + 1, pos.getZ() + 1).expand(this.getSpawnRange())).size();
    ...
    private void updateSpawns(World world, BlockPos pos) {
        ...
        this.spawnDelay = this.getMaxSpawnDelay() <= this.getMinSpawnDelay() ? this.getMinSpawnDelay() : this.getMinSpawnDelay() + random.nextInt(this.getMaxSpawnDelay() - this.getMinSpawnDelay());
    ...
    public NbtCompound writeNbt(NbtCompound nbt) {
        ...
        nbt.putShort("MinSpawnDelay", (short)this.getMinSpawnDelay());
        nbt.putShort("MaxSpawnDelay", (short)this.getMaxSpawnDelay());
        nbt.putShort("SpawnCount", (short)this.getSpawnCount());
        nbt.putShort("MaxNearbyEntities", (short)this.getMaxNearbyEntities());
        nbt.putShort("RequiredPlayerRange", (short)this.getRequiredPlayerRange());
        nbt.putShort("SpawnRange", (short)this.getSpawnRange());
